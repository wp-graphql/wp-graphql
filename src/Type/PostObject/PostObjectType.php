...
class PostObjectType extends WPObjectType {
	...
	public function __construct( $post_type ) {
		 ...
		 * Get the Type name
		 */
		self::$type_name = ucfirst( self::$post_type_object->graphql_single_name );

		$config = [
			'name'        => self::$type_name,
			// translators: the placeholder is the post_type of the object
			'description' => sprintf( __( 'The %s object type', 'wp-graphql' ), self::$type_name ),
			'fields' => function() {
				return self::fields();
			},
	...
	protected static function fields() {
		 ...
		 * @since 0.0.5
		 */
		if ( empty( self::$fields[ self::$type_name ] ) ) :
			/**
			 ...
			 * @since 0.0.5
			 */
			if ( post_type_supports( self::$post_type_object->name, 'comments' ) ) {
			 ...
			 */
			if ( true === self::$post_type_object->hierarchical ) {
				$fields[ 'child' . ucfirst( self::$post_type_object->graphql_plural_name ) ] = PostObjectConnectionDefinition::connection( self::$post_type_object );
			}
			...
			if ( ! empty( $allowed_taxonomies ) && is_array( $allowed_taxonomies ) ) {
				foreach ( $allowed_taxonomies as $taxonomy ) {
					...
					if ( in_array( $taxonomy, get_object_taxonomies( self::$post_type_object->name ), true ) ) {
			...
			if ( post_type_supports( self::$post_type_object->name, 'thumbnail' ) ) {
		...
		return ! empty( self::$fields[ self::$type_name ] ) ? self::$fields[ self::$type_name ] : null;
	}
