...
class NegativeInt {
	...
	private static function coerce( $value ) {
		if ( ! is_numeric( $value ) || $value > PHP_INT_MAX ) {
			...
			throw new InvariantViolation( sprintf( __( 'NegativeInt cannot represent non-numeric value: %s', 'wp-graphql' ), Utils::printSafe( $value ) ) );
		}
		...
		if ( $intValue != $value ) { // phpcs:ignore
			// translators: %s is the value passed to the scalar
			throw new InvariantViolation( sprintf( __( 'NegativeInt cannot represent non-integer value: %s', 'wp-graphql' ), Utils::printSafe( $value ) ) );
		}

		if ( $intValue >= 0 ) {
			// translators: %s is the value passed to the scalar
			throw new InvariantViolation( sprintf( __( 'NegativeInt cannot represent non-negative value: %s', 'wp-graphql' ), Utils::printSafe( $value ) ) );
		}
		return $intValue;
	}
	...
	public static function register_scalar() {
		register_graphql_scalar(
			'NegativeInt',
			[
				'description'    => __( 'Integers that will have a value less than 0.', 'wp-graphql' ),
				'serialize'      => [ self::class, 'serialize' ],
				'parseValue'     => [ self::class, 'parseValue' ],
				'parseLiteral'   => [ self::class, 'parseLiteral' ],
				'specifiedByURL' => 'https://www.w3.org/TR/xmlschema-2/#negativeInteger',
			]
		);
	}
}