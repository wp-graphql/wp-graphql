			...
			throw new InvariantViolation(
				esc_html(
					sprintf(
						...
						__( 'NonPositiveInt cannot represent non-numeric value: %s', 'wp-graphql' ),
		...
		if ( $intValue != $value ) { // phpcs:ignore
			throw new InvariantViolation(
				esc_html(
					sprintf(
						/* translators: %s: The value that was passed to be serialized */
						__( 'NonPositiveInt cannot represent non-integer value: %s', 'wp-graphql' ),
						Utils::printSafe( $value )
					)
				)
			);
		...
		if ( $intValue > 0 ) {
			throw new InvariantViolation(
				esc_html(
					sprintf(
						/* translators: %s: The value that was passed to be serialized */
						__( 'NonPositiveInt cannot represent positive value: %s', 'wp-graphql' ),
						Utils::printSafe( $value )
					)
				)
			);
