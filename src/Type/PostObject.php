...
class PostObject {
	/**
	 * @param \WP_Post_Type $post_type_object The post_type_object for the post_type being added to the Schema
	 */
	public static function register_type( $post_type_object ) {
		if ( ! empty( $post_type_object->graphql_single_name ) ) {

			register_graphql_object_type( ucfirst( $post_type_object->graphql_single_name ), [
				'description' => sprintf( __( 'The %s object type', 'wp-graphql' ), $post_type_object->graphql_single_name ),
				...
				'fields'      => [
					...
					'terms'             => [
						...
						'description' => sprintf( __( 'Terms connected to the %1$s', 'wp-graphql' ), $post_type_object->graphql_single_name ),
						'resolve'     => function ( \WP_Post $post, $args, AppContext $context, ResolveInfo $info ) {
