...
class TermObjectType extends WPObjectType {
	...
	public function __construct( $taxonomy ) {
		 ...
		 * Set the type name
		 */
		self::$type_name = ucfirst( self::$taxonomy_object->graphql_single_name );
	...
	protected static function fields() {
		...
		if ( empty( self::$fields[ $single_name ] ) ) :
			 ...
			 * @since 0.0.5
			 */
			self::$fields[ $single_name ] = [
				'id'                => [
					'type'        => Types::non_null( Types::id() ),
					# Placeholder is the name of the taxonomy
					'description' => __( 'The global ID for the ' . self::$taxonomy_object->name, 'wp-graphql' ),
			 ...
			 */
			if ( true === self::$taxonomy_object->hierarchical ) {
				$fields['parent'] = [
					'type'        => Types::term_object( self::$taxonomy_object->name ),
				...
				$fields['ancestors'] = [
					'type'        => Types::list_of( Types::term_object( self::$taxonomy_object->name ) ),
				...
				$fields['children'] = TermObjectConnectionDefinition::connection( self::$taxonomy_object );
			...
			if ( ! empty( $allowed_post_types ) && is_array( $allowed_post_types ) ) {
				foreach ( $allowed_post_types as $post_type ) {
					if ( in_array( $post_type, self::$taxonomy_object->object_type, true ) ) {
