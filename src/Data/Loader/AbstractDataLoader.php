...
abstract class AbstractDataLoader {
	...
	public function load( $key ) {
		...
		return isset( $result[ $key ] ) ? $this->get_normalized_model( $result[ $key ], $key ) : null;
	}
	 ...
	 * @return null|Model
	 */
	public function get_normalized_model( $entry, $key ) {
		 ...
		 */
		$pre_loaded_model = apply_filters( 'graphql_dataloader_get_model', null, $entry, $key, $this );

		/**
		 * If a Model has been pre-loaded via filter, return it and skip the
		 *
		 */
		if ( ! empty( $pre_loaded_model ) ) {
		 ...
		 * @param AbstractDataLoader $this  The AbstractDataLoader Instance
		 */
		return apply_filters( 'graphql_dataloader_get_normalized_model', $model, $entry, $key, $this );
	}
