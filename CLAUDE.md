...
- **Schema descriptions describe the API, not the WordPress plumbing.** Field / argument / type `description` strings are the public API contract, so write them in domain / GraphQL terms and keep WordPress implementation internals out of them: no post meta keys (`_wp_page_template`, `_thumbnail_id`), WP class properties (`WP_Post->guid`), DB tables/columns (the `post_objects` table, the `post_mime_type` column), function names (`wp_*`, `WP_Query`), or query-var names. If you need to convey how something is identified, describe what the user provides (a template file name, a slug, an ID), not where it is stored. Referencing WordPress *concepts* by their user-facing name (sticky posts, taxonomies, menus) is fine, the rule targets plumbing, not the CMS. Code comments and developer-facing deprecation messages may still reference internals, this applies to the user-facing `description` strings only.
