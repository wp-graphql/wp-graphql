...
# Go up from plugins/wp-graphql-pqc to root
...
echo "=========================================="
echo "WPGraphQL PQC Comprehensive Test"
echo "=========================================="
echo "Running from: $(pwd)"
echo ""

# PQC only stores when Smart Cache / Query Analyzer produces X-GraphQL-Keys (cache key list).
# Without that, PostHandler skips storage. Force the setting on (Smart Cache normally does this).
echo "=== Prerequisites: Query Analyzer + PQC tables ==="
...
$active = is_plugin_active( "wp-graphql-pqc/wp-graphql-pqc.php" );
$schema_ok = class_exists( "\\WPGraphQL\\PQC\\Database\\Schema" ) && \WPGraphQL\PQC\Database\Schema::table_exists();
echo "pqc_active=" . ( $active ? "yes" : "no" ) . " tables=" . ( $schema_ok ? "yes" : "no" ) . "\n";
if ( ! $active ) {
	echo "WARNING: Activate wp-graphql-pqc in wp-admin or: wp plugin activate wp-graphql-pqc\n";
}
if ( ! $schema_ok && class_exists( "\\WPGraphQL\\PQC\\Database\\Schema" ) ) {
	\WPGraphQL\PQC\Database\Schema::create_table();
...
npm run wp-env -- run cli -- wp rewrite list 2>/dev/null | grep -F 'graphql/persisted' || echo "WARNING: No graphql/persisted rules found — PQC GET URLs will not work."
echo ""
...
# Usage: echo "$raw" | pqc_json_get "extensions.persistedQueryNonce"
pqc_json_get() {
    local dot_path="$1"
    PQC_JSON_PATH="$dot_path" python3 -c "
import json, sys, os
raw = sys.stdin.read()
path = os.environ.get('PQC_JSON_PATH', '').split('.')
...
get_nonce() {
    ...
    http_code=$(curl -sS --compressed -o "$tmp" -w "%{http_code}" -X GET "$url" \
        ...
        -H "User-Agent: wp-graphql-pqc-test/1" || echo "000")
    local body
    body=$(cat "$tmp" 2>/dev/null || true)
    rm -f "$tmp"
    
    local nonce
    nonce=$(echo "$body" | pqc_json_get "extensions.persistedQueryNonce")
...
verify_warm_persisted_get() {
    ...
    http_code=$(curl -sS --compressed -o "$body" -D "$hdr" -w "%{http_code}" -X GET "$url" \
        -H "Accept: application/json" \
        -H "User-Agent: wp-graphql-pqc-test/warm-get" || echo "000")
    ...
    local data_json
    data_json=$(echo "$body_str" | pqc_json_get "data")
    ...
    local warm_nonce
    warm_nonce=$(echo "$body_str" | pqc_json_get "extensions.persistedQueryNonce")
...
persist_query() {
    ...
    local gql_errors
    gql_errors=$(echo "$response" | pqc_json_get "errors")
    ...
    local persisted_url
    persisted_url=$(echo "$response" | pqc_json_get "extensions.persistedQueryUrl")
    local cache_keys
    cache_keys=$(echo "$response" | pqc_json_get "extensions.queryAnalyzer.keys")
    
    if [ -z "$persisted_url" ]; then
        echo "ERROR: extensions.persistedQueryUrl missing — PQC did not index this request." >&2
...
if [ -z "$POST_EXISTS" ]; then
    ...
    POST_ID=$(npm run wp-env -- run cli -- wp post create \
        ...
        --post_content="This is test post 9 for PQC testing" \
        --post_status=publish \
        --porcelain)
...
echo "=========================================="
echo ""
echo "Clearing PQC index tables so GET /graphql/persisted/{hash} returns a nonce (cold miss)."
...
if [ -n "$DEBUG_LOG" ] && npm run wp-env -- run cli -- test -f "$DEBUG_LOG" 2>/dev/null; then
    echo "--- Recent PQC purge events from debug.log ---"
    npm run wp-env -- run cli -- tail -n 100 "$DEBUG_LOG" 2>/dev/null | grep "WPGraphQL PQC" || echo "No PQC events found in recent logs"
    ...
    for log_path in "${DEBUG_LOG_PATHS[@]}"; do
        if npm run wp-env -- run cli -- test -f "$log_path" 2>/dev/null; then
            ...
            npm run wp-env -- run cli -- tail -n 100 "$log_path" 2>/dev/null | grep "WPGraphQL PQC" || echo "No PQC events found"
            break
        fi
    done
fi
...
echo "  add_filter( 'wpgraphql_pqc_delete_entries_on_purge', '__return_false' );"
echo ""
