# PQC edge-cache benchmarking (plan summary)

**Goal:** Reproduce locally: edge cache in front of WordPress, persisted PQC GET URLs, Smart Cache driven purges, and k6 load so we can compare origin load and latency with high edge TTLs vs content churn.
...
- `HttpPurgeAdapter` + env for purge target; PQC `delete_by_url` after edge eviction; executions kept for warm GET.
