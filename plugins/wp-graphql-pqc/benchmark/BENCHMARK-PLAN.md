...
### PQC plugin — normalized URL ↔ cache key map (MySQL)

The denormalized `wpgraphql_pqc_url_keys` table is **replaced** by a normalized layout (see [INTEGRATIONS.md](../docs/INTEGRATIONS.md), [Schema.php](../src/Database/Schema.php)):

- **`wpgraphql_pqc_urls`** — one row per persisted path; `last_seen_at` for GC.
- **`wpgraphql_pqc_cache_keys`** — deduplicated analyzer key strings.
- **`wpgraphql_pqc_key_urls`** — junction (cache key ↔ URL).

**Executions** (`wpgraphql_pqc_executions`) remain for warm GET resolution independent of the key map. Legacy installs migrate on init/activate; executions are backfilled before dropping old `url_keys` data.

**Behavior highlights:**

- **`store( …, $record_cache_tags )`** — can skip key-map writes while still storing document/execution; **`wpgraphql_pqc_should_record`** defaults to **`! is_user_logged_in()`** so the map tracks publicly cacheable edge URLs.
...
1. **Paired runs (same knobs)** — Run **`run-k6-origin.sh`** and **`run-k6-edge.sh`** with identical `DURATION`, `VUS`, and `urls.txt`. Capture outputs using [RESULTS.template.md](./RESULTS.template.md) + [k6/run-manifest.example.json](./k6/run-manifest.example.json).
