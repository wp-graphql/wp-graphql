...
const selectors = {
	...
	checkboxA:
		'#wpuf-graphql_section_a_settings\\[graphql_section_a_checkbox\\]',
	checkboxB:
		'#wpuf-graphql_section_b_settings\\[graphql_section_b_checkbox\\]',
};
...
describe('Settings Page', () => {
	...
	test('Verify custom plugin is active and tabs are present', async ({
		page,
	}) => {
	...
	test('Switch between tabs and verify visibility', async ({ page }) => {
		...
		await expect(page.locator(selectors.navTabGeneral)).toHaveClass(
			/nav-tab-active/
		);
		...
		await page.waitForTimeout(500);
		await expect(page.locator(selectors.navTabA)).toHaveClass(
			/nav-tab-active/
		);
		...
		await page.waitForTimeout(500);
		await expect(page.locator(selectors.navTabB)).toHaveClass(
			/nav-tab-active/
		);
	...
	test('Verify checkbox functionality in Section A and B', async ({
		page,
	}) => {
		await visitAdminFacingPage(
			page,
