...
describe('Settings Registry Options Page', () => {
    ...
    test('Updating WPGraphQL options via options.php does not cause fatal errors', async ({ page }) => {
        ...
        const graphqlExperimentsInput = page.locator('input[name="graphql_experiments_settings"]');
        if (await graphqlExperimentsInput.count() > 0) {
            ...
            await expect(page.locator('body')).not.toContainText('sanitize_options(): Argument #1 ($options) must be of type array');
        }
        ...
        const graphqlGeneralInput = page.locator('input[name="graphql_general_settings"]');
