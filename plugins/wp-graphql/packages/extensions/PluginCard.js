...
const PluginCard = ({ plugin }) => {
		...
		try {
			...
			window.wpgraphqlExtensions.extensions =
				window.wpgraphqlExtensions.extensions.map((extension) =>
					extension.plugin_url === plugin.plugin_url
						? {
								...extension,
	...
	const { buttonText, buttonDisabled } = getButtonDetails(
		host,
		plugin.plugin_url,
		isInstalled,
		isActive,
		...
		<div className="plugin-card">
			<div className="plugin-card-top">
				<div className="name column-name">
					...
					{plugin.experiment && (
						<em className="plugin-experimental">(experimental)</em>
					)}
				</div>
