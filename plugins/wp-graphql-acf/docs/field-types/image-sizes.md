...
-   **ACFE\_Image\_Size** (object type): An object type representing a registered image size in WordPress, with fields such as height, width, name and crop.

## Resolve Type

Fields of the `acfe_image_sizes` type will resolve to a "List of ACFE\_Image\_Size" in the GraphQL Schema.
...
An example of registering a field group with a `acfe_image_sizes` field in PHP (the same can be configured in the ACF admin UI, or via ACF JSON):
...
add_action( 'acf/include_fields', function() {
	...
	acf_add_local_field_group( [
		'key'                              => 'my_field_group_acfe_image_sizes',
...
```graphql
query ImageSizesField($uri: String! = "kitchen-sink") {
  nodeByUri(uri: $uri) {
    id
    uri
...
fragment WithAcfAcfExtendedProKitchenSink on WithAcfAcfExtendedProKitchenSink {
