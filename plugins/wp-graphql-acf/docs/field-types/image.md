...
An example of registering a field group with a `image` field in PHP (the same can be configured in the ACF admin UI, or via ACF JSON):
...
```graphql
query ImageField($uri: String! = "kitchen-sink") {
  nodeByUri(uri: $uri) {
    id
    uri
      ...
      "acfFreeKitchenSink": {
        "image": {
          "node": {
            ...
            "id": "cG9zdDozNjE=",
            "mediaItemUrl": "https://bpacfwpgraphql.wpengine.com/wp-content/uploads/2023/05/logo-wpgraphql.png"
          }
        }
      }
    }
