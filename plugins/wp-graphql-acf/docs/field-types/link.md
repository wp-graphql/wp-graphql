...
An example of registering a field group with a `link` field in PHP (the same can be configured in the ACF admin UI, or via ACF JSON):
...
## Querying the Link field

```graphql
query LinkField($uri: String! = "kitchen-sink") {
  nodeByUri(uri: $uri) {
    id
    uri
  ...
  "data": {
    "nodeByUri": {
      ...
      "acfFreeKitchenSink": {
        "link": {
          "__typename": "AcfLink",
          "url": "https://hbc1ouaoosky2r11opkrmug9k.js.wpenginepowered.com/kitchen-sink/",
          "title": "Kitchen Sink",
          "target": ""
        }
      }
