...
The Color Picker field type is native to ACF (free) and provides users with the option to select a color using a Color Picker.

## Resolve Type

Fields of the "color\_picker" field type resolve to "String" in the GraphQL Schema.
...
An example of registering a field group with a `color_picker` field in PHP (the same can be configured in the ACF admin UI, or via ACF JSON):
...
add_action( 'acf/include_fields', function() {
	...
	acf_add_local_field_group( [
		'key'                              => 'my_field_group_color_picker',
		'title'                            => 'My Field Group with color_picker',
		'show_in_graphql'                  => 1,
		'graphql_field_name'               => 'myFieldGroupWithColorPicker',
		'map_graphql_types_from_location_rules' => 0,
		'graphql_types'                    => [ 'Page' ],
		'fields'                           => [
			[
				'key'                => 'my_field_color_picker',
				'label'              => 'My Field',
				'name'               => 'my_field',
				'type'               => 'color_picker',
				'show_in_graphql'    => 1,
				'graphql_field_name' => 'myFieldWithColorPicker',
			],
		],
		'location'                         => [
			[
...
## Querying the Color Picker Field

```graphql
query ColorPickerField($uri: String! = "kitchen-sink") {
  nodeByUri(uri: $uri) {
    id
    uri
