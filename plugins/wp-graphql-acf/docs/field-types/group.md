...
Each ACF Field Group is added to the GraphQL Schema as an object type, named using the "graphql\_field\_name" or "graphql\_type\_name" field on the field group, implementing the `AcfFieldGroup` Interface, and containing fields representing the fields of the group.
...
An example of registering a field group with a `group` field in PHP (the same can be configured in the ACF admin UI, or via ACF JSON):
...
```graphql
query GroupField($uri: String! = "kitchen-sink") {
  nodeByUri(uri: $uri) {
    id
    uri
...
fragment WithAcfFreeProKitchenSink on WithAcfAcfFreeKitchenSink {
  acfFreeKitchenSink {
    group {
      ...AcfFreeKitchenSinkGroup
    }
  }
}

fragment AcfFreeKitchenSinkGroup on AcfFreeKitchenSinkGroup {
	nestedTextField
}
```
...
```json
{
  "data": {
    "nodeByUri": {
      ...
      "acfFreeKitchenSink": {
        "group": {
          "nestedTextField": "This is a value in a text field nested within another field group..."
        }
      }
    }
  }
