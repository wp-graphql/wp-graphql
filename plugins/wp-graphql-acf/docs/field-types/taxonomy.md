...
| `add_term` | Allow new terms to be created whilst editing | This setting modifies editorial behavior in the admin and does not impact the GraphQL Schema or GraphQL resolvers. |
...
An example of registering a field group with a `taxonomy` field in PHP (the same can be configured in the ACF admin UI, or via ACF JSON):
...
```graphql
query TaxonomyField($uri: String! = "kitchen-sink") {
  nodeByUri(uri: $uri) {
    id
    uri
