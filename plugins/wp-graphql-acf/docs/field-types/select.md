...
An example of registering a field group with a `select` field in PHP (the same can be configured in the ACF admin UI, or via ACF JSON):
...
```graphql
query SelectField($uri: String! = "kitchen-sink") {
  nodeByUri(uri: $uri) {
    id
    uri
