...
An example of registering a field group with a `true_false` field in PHP (the same can be configured in the ACF admin UI, or via ACF JSON):
...
```graphql
query TrueFalseField($uri: String! = "kitchen-sink") {
  nodeByUri(uri: $uri) {
    id
    uri
