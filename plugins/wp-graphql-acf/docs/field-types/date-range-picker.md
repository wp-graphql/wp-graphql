...
The Date Range Picker field is made available by the Advanced Custom Fields Extended PRO plugin and provides users with a Date Range Picker using an enhanced version of the [Daterangepicker library](http://www.daterangepicker.com/).

## Types Added to the Schema

To support the Date Range Picker field, an additional Object Type is added to the Schema.

-   **ACFE\_Date\_Range** (object type): Object representing a range of dates including fields for startDate and endDate.

## Resolve Type

The "date\_range\_picker" field type resolves as "ACFE\_Date\_Range" in the GraphQL Schema with startDate and endDate fields.
...
An example of registering a field group with a `acfe_date_range_picker` field in PHP (the same can be configured in the ACF admin UI, or via ACF JSON):
...
add_action( 'acf/include_fields', function() {
	...
	acf_add_local_field_group( [
		'key'                              => 'my_field_group_acfe_date_range_picker',
		...
		'fields'                           => [
			[
				'key'                => 'my_field_acfe_date_range_picker',
				'label'              => 'My Field',
				'name'               => 'my_field',
...
```graphql
query DateRangePickerField($uri: String! = "kitchen-sink") {
  nodeByUri(uri: $uri) {
    id
    uri
...
fragment WithAcfFreeProKitchenSink on WithAcfAcfExtendedProKitchenSink {
    ...
    "nodeByUri": {
      ...
      "acfExtendedProKitchenSink": {
        "dateRangePicker": {
          ...
          "endDate": "2024-02-04T00:00:00+00:00"
        }
      }
    }
  }
