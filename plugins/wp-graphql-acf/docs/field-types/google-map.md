...
The "google\_map" field type resolves as "AcfGoogleMap" in the GraphQL Schema with latitude, longitude, and other fields.

## Requirements

In order to use the Google Map field with ACF you must have a Google API key setup. You can read more about that in the [ACF Google Map field documentation](https://www.advancedcustomfields.com/resources/google-map/#requirements).
...
An example of registering a field group with a `google_map` field in PHP (the same can be configured in the ACF admin UI, or via ACF JSON):
...
```graphql
query GoogleMapField($uri: String! = "kitchen-sink") {
  nodeByUri(uri: $uri) {
    id
    uri
...
fragment AcfGoogleMap on AcfGoogleMap {
  latitude
  longitude
  city
  state
