...
| `toggle` | Prepend an extra checkbox to toggle all choices | This setting modifies editorial behavior in the admin and does not impact the GraphQL Schema or GraphQL resolvers. |
...
An example of registering a field group with a `acfe_user_roles` field in PHP (the same can be configured in the ACF admin UI, or via ACF JSON):
...
add_action( 'acf/include_fields', function() {
	...
	acf_add_local_field_group( [
		'key'                              => 'my_field_group_acfe_user_roles',
