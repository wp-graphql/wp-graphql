...
The DateTime Picker field is native to ACF (free) and provides users with a jQuery date selection popup interface to pick a date and time.

## Resolve Type

The "date\_time\_picker" field type resolves as a "String" in the GraphQL Schema.

The date and time will be returned in the [RFC3339](https://www.rfc-editor.org/rfc/rfc3339.html) format.
...
An example of registering a field group with a `date_time_picker` field in PHP (the same can be configured in the ACF admin UI, or via ACF JSON):
...
add_action( 'acf/include_fields', function() {
	...
	acf_add_local_field_group( [
		'key'                              => 'my_field_group_date_time_picker',
		'title'                            => 'My Field Group with date_time_picker',
		'show_in_graphql'                  => 1,
		'graphql_field_name'               => 'myFieldGroupWithDateTimePicker',
		'map_graphql_types_from_location_rules' => 0,
		'graphql_types'                    => [ 'Page' ],
		'fields'                           => [
			[
				'key'                => 'my_field_date_time_picker',
				'label'              => 'My Field',
				'name'               => 'my_field',
				'type'               => 'date_time_picker',
				'show_in_graphql'    => 1,
				'graphql_field_name' => 'myFieldWithDateTimePicker',
			],
		],
		'location'                         => [
			[
...
## Querying the DateTime Picker field

```graphql
query DateTimePickerField($uri: String! = "kitchen-sink") {
  nodeByUri(uri: $uri) {
    id
    uri
...
```json
{
  "data": {
    "nodeByUri": {
      ...
      "uri": "/kitchen-sink/",
      "acfFreeKitchenSink": {
        "dateTimePicker": "2023-05-26T00:00:00+00:00"
      }
    }
  }
}
