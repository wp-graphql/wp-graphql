...
Fields of the `acfe_currencies` field type resolve to a "List of ACFE\_Currency" in the GraphQL Schema. This supports singular and multiple selections as a list can be either one or more than one values.
...
-   **ACFE\_Currency** (object type): The Object Type representing a Currency with fields such as "name", "continent", "code" and more.
...
| `Group by Continents` |  |  |
| `Display Flags` |  |  |
| `Display Format` | The format displayed when editing a post | This is a presentational field in the WordPress admin and has no impact on the GraphQL Schema or GraphQL resolvers. |
...
| `ajax` | Trigger ajax event on click. [See documentation](https://www.acf-extended.com/features/fields/button) | This is a presentational field in the WordPress admin and has no impact on the GraphQL Schema or GraphQL resolvers. |
...
| `allow_custom` | Allow 'custom' values to be added | This setting modifies editorial behavior in the admin and does not impact the GraphQL Schema or GraphQL resolvers. |
...
An example of registering a field group with a `acfe_currencies` field in PHP (the same can be configured in the ACF admin UI, or via ACF JSON):
...
add_action( 'acf/include_fields', function() {
	...
	acf_add_local_field_group( [
		'key'                              => 'my_field_group_acfe_currencies',
...
```graphql
query CurrenciesField($uri: String! = "kitchen-sink") {
  nodeByUri(uri: $uri) {
    id
    uri
    ...WithAcfExtendedProKitchenSink
  }
}

fragment WithAcfExtendedProKitchenSink on WithAcfAcfExtendedProKitchenSink {
  acfExtendedProKitchenSink {
    acfeCurrency {
      ...ACFE_Currency
    }
  }
...
fragment ACFE_Currency on ACFE_Currency {
  __typename
  name
  continent
}
