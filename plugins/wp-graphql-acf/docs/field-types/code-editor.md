...
Fields of the "acfe\_code\_editor" field type resolve to a "String" in the GraphQL Schema.
...
| `indent_unit` | How many spaces a block (whatever that means in the edited language) should be indented |  |
| `lines` | Whether to show line numbers to the left of the editor |  |
| `mode` | Choose the syntax highlight |  |
...
An example of registering a field group with a `acfe_code_editor` field in PHP (the same can be configured in the ACF admin UI, or via ACF JSON):
...
add_action( 'acf/include_fields', function() {
	...
	acf_add_local_field_group( [
		'key'                              => 'my_field_group_acfe_code_editor',
...
```graphql
query CodeEditorField($uri: String! = "kitchen-sink") {
  nodeByUri(uri: $uri) {
    id
    uri
    ...WithAcfExtendedFreeKitchenSink
  }
}

fragment WithAcfExtendedFreeKitchenSink on WithAcfAcfExtendedFreeKitchenSink {
