...
Fields of the `acfe_countries` field type resolve to a "List of ACFE\_Country" in the GraphQL Schema. This supports singular and multiple selections as a list can be either one or more than one values.
...
-   **ACFE\_Country** (object type): The Object Type representing a Country with fields such as "name", "continent", "code" and more.
...
## Querying the Countries field

```graphql
query CountriesField($uri: String! = "kitchen-sink") {
  nodeByUri(uri: $uri) {
    id
    uri
...
fragment ACFE_Country on ACFE_Country {
  __typename
  name
  continent
}
