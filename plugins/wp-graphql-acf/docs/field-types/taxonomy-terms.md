...
An example of registering a field group with a `acfe_taxonomy_terms` field in PHP (the same can be configured in the ACF admin UI, or via ACF JSON):
...
add_action( 'acf/include_fields', function() {
	...
	acf_add_local_field_group( [
		'key'                              => 'my_field_group_acfe_taxonomy_terms',
		...
		'fields'                           => [
			[
				'key'                => 'my_field_acfe_taxonomy_terms',
				'label'              => 'My Field',
				'name'               => 'my_field',
...
```graphql
query TaxonomyTermsField($uri: String! = "kitchen-sink") {
  nodeByUri(uri: $uri) {
    id
    uri
