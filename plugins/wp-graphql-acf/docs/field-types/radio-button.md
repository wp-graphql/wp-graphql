...
The Radio Button field type is native to ACF (free) and provides a list of select-able inputs.

## Resolve Type

Fields of the "radio\_button" field type resolve to a "String" in the GraphQL Schema. (even if the choice values are integers or booleans)
...
An example of registering a field group with a `radio` field in PHP (the same can be configured in the ACF admin UI, or via ACF JSON):
...
add_action( 'acf/include_fields', function() {
	...
	acf_add_local_field_group( [
		'key'                              => 'my_field_group_radio',
		'title'                            => 'My Field Group with radio',
		'show_in_graphql'                  => 1,
		'graphql_field_name'               => 'myFieldGroupWithRadio',
		...
		'fields'                           => [
			[
				...
				'type'               => 'radio',
				'show_in_graphql'    => 1,
				'graphql_field_name' => 'myFieldWithRadio',
			],
		],
		'location'                         => [
			[
...
## Querying the Radio Button Field

```graphql
query RadioField($uri: String! = "kitchen-sink") {
  nodeByUri(uri: $uri) {
    id
    uri
