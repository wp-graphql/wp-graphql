...
const SKIP_FIELD_TYPES_GRAPHQL = [
	'acfe_date_range_picker',
	'acfe_currencies',
	'acfe_countries',
];
...
function getStorageStatePath() {
	...
	const artifacts =
		...
		path.join(
			cwd.endsWith('wp-graphql-acf')
				? cwd
				: path.join(cwd, 'plugins', 'wp-graphql-acf'),
			'artifacts'
		);
	return (
		process.env.STORAGE_STATE_PATH ??
		path.join(artifacts, 'storage-states', 'admin.json')
	);
}
...
test.describe('Field types GraphQL UI', () => {
	for (const config of FIELD_TYPE_CONFIGS) {
		...
		const needsAcfPro =
			ACF_PRO_ONLY_TYPES.includes(config.type) || isAcfeType;
		...
		describeField(`${config.type} field`, () => {
			...
			beforeEach(async ({ page }) => {
				const fieldGroupTitle =
					config.fieldGroupTitle ?? FIELD_GROUP_TITLE;
			...
			const runGraphQLTabTest = SKIP_FIELD_TYPES_GRAPHQL.includes(
				config.type
			)
				? test.skip
				: test;
			runGraphQLTabTest(
				'GraphQL tab: visibility and save of Show in GraphQL, Description, Field Name',
				async ({ page }) => {
					...
					await expect(
						panel
							.locator(
								'[data-name="show_in_graphql"] .description'
							)
							.getByText(/breaking change/i)
					).toBeVisible({ timeout: visibilityTimeout });

					// 2) Save Show in GraphQL (uncheck then check + field name)
					const showInGraphql = panel
						.locator('[data-name="show_in_graphql"]')
						.first();
					const switchEl = showInGraphql
						.locator('.acf-switch')
						.first();
					if ((await switchEl.count()) > 0) {
						await switchEl.click();
					} else {
						const checkbox = showInGraphql.locator(
							'input[type="checkbox"]'
						);
					...
					await expect(
						page.locator('#message.notice-error')
					).not.toBeVisible();
					await expect(
					...
					const checkbox2 = panel.locator(
						'[data-name="show_in_graphql"] input[type="checkbox"]'
					);
					if ((await checkbox2.count()) > 0) {
						await expect(checkbox2).not.toBeChecked();
					}

					const showInGraphql2 = panel
						.locator('[data-name="show_in_graphql"]')
						.first();
					const switchEl2 = showInGraphql2
						.locator('.acf-switch')
						.first();
					...
					const fieldNameInputStep2 = panel.locator(
						'[data-name="graphql_field_name"] input[type="text"]'
					);
					...
					await expect(
						panel
							.locator('[data-name="graphql_description"]')
							.getByText('GraphQL Description')
					).toBeVisible();
					const descInput = panel
						.locator(
							'[data-name="graphql_description"] input[type="text"]'
						)
						.first();
					...
					await expect(
						panel
							...
							.first()
					).toHaveValue('test description...', { timeout: 10000 });
					...
					await expect(
						panel
							.locator('[data-name="graphql_field_name"]')
							.getByText('GraphQL Field Name')
					).toBeVisible();
					const fieldNameInput = panel.locator(
						'[data-name="graphql_field_name"] input[type="text"]'
					);
