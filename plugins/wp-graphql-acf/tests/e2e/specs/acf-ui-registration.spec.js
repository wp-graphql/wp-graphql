...
async function deleteAcfUiEntity(page, listUrl, rowLabel) {
	...
	await visitAdminFacingPage(
		page,
		`${wpAdminUrl}/${listUrl}&post_status=trash`
	);
	const trashTable = page.locator('.wp-list-table').first();
	const trashRow = trashTable
		.locator('tbody tr:not(.no-items)')
		.filter({ hasText: rowLabel })
		.first();
	if ((await trashRow.count()) === 0) {
		return;
	}
...
describeAcfUi('ACF UI Registration – Custom Post Type', () => {
	...
	test('post type can be registered to show in GraphQL', async ({ page }) => {
		await visitAdminFacingPage(
			page,
			`${wpAdminUrl}/edit.php?post_type=acf-post-type`
		);
		await expect(
			page.getByText('Post Types', { exact: true }).first()
		).toBeVisible();
		await page
			.locator('.acf-headerbar')
			.getByRole('link', { name: /add new/i })
			.first()
			.click();
		await expect(
			page.getByRole('heading', { name: 'Add New Post Type' }).first()
		).toBeVisible();

		await page
			.locator('label[for="acf_post_type-advanced_configuration"]')
			.click();
		await expect(page.locator('#acf-advanced-settings')).toBeVisible();
		await page
			.locator('#acf-advanced-settings')
			.getByRole('link', { name: 'GraphQL' })
			.click();

		await expect(
			page.locator('#acf_post_type-show_in_graphql')
		).not.toBeChecked();
		await page
		...
		await page
			...
			.fill('Test Types');
		await page
			.locator('input[name="acf_post_type[post_type]"]')
			.fill('test_type');
		await page
			.locator('label[for="acf_post_type-show_in_graphql"]')
			.click();
		...
		await expect(
			page.locator('input[name="acf_post_type[labels][singular_name]"]')
		).toHaveValue('Test Type');
		await expect(
			page.locator('input[name="acf_post_type[graphql_single_name]"]')
		).toHaveValue('testSingleName');
		await expect(
			page.locator('input[name="acf_post_type[graphql_plural_name]"]')
		).toHaveValue('testPluralName');

		await visitAdminFacingPage(page, TOOLS_URL);
		await expect(page.getByText('Select Post Types')).toBeVisible();
		await expect(
			page.locator('[data-name="post_type_keys"]').getByText('Test Types')
		).toBeVisible();
		await page
			.locator('[data-name="post_type_keys"]')
			.getByRole('checkbox', { name: 'Test Types' })
			.check();
		await page
			.locator('#acf-admin-tool-export')
			.getByRole('button', { name: 'Generate PHP' })
			.click();
		...
		expect(phpContent).toContain(
			"'graphql_single_name' => 'testSingleName'"
		);
		expect(phpContent).toContain(
			"'graphql_plural_name' => 'testPluralName'"
		);
	});
});
...
describeAcfUi('ACF UI Registration – Custom Taxonomy', () => {
	...
	test('taxonomy can be registered to show in GraphQL', async ({ page }) => {
		await visitAdminFacingPage(
			page,
			`${wpAdminUrl}/edit.php?post_type=acf-taxonomy`
		);
		await expect(
			page.getByText('Taxonomies', { exact: true }).first()
		).toBeVisible();
		await page
			.locator('.acf-headerbar')
			.getByRole('link', { name: /add new/i })
			...
			.click();
		await expect(
			page.getByRole('heading', { name: 'Add New Taxonomy' }).first()
		).toBeVisible();

		await page
			.locator('label[for="acf_taxonomy-advanced_configuration"]')
			.click();
		await expect(page.locator('#acf-advanced-settings')).toBeVisible();
		await page
			.locator('#acf-advanced-settings')
		...
		await expect(
			page.locator('#acf_taxonomy-show_in_graphql')
		).not.toBeChecked();
		await page
			.locator('input[name="acf_taxonomy[labels][singular_name]"]')
			.fill('Test Type');
		await page
			.locator('input[name="acf_taxonomy[labels][name]"]')
			.fill('Test Types');
		await page
			.locator('input[name="acf_taxonomy[taxonomy]"]')
			.fill('test_type');
		...
		await expect(
			page.locator('input[name="acf_taxonomy[labels][singular_name]"]')
		).toHaveValue('Test Type');
		await expect(
			page.locator('input[name="acf_taxonomy[graphql_single_name]"]')
		).toHaveValue('testSingleName');
		await expect(
			page.locator('input[name="acf_taxonomy[graphql_plural_name]"]')
		).toHaveValue('testPluralName');

		await visitAdminFacingPage(page, TOOLS_URL);
		await expect(page.getByText('Select Taxonomies')).toBeVisible();
		await expect(
			page.locator('[data-name="taxonomy_keys"]').getByText('Test Types')
		).toBeVisible();
		await page
			.locator('[data-name="taxonomy_keys"]')
			.getByRole('checkbox', { name: 'Test Types' })
			.check();
		await page
			.locator('#acf-admin-tool-export')
...
describeAcfUi('ACF UI Registration – Options Page', () => {
	...
	test('options page can be registered to show in GraphQL', async ({
		page,
	}) => {
		await visitAdminFacingPage(
			page,
			`${wpAdminUrl}/edit.php?post_type=acf-ui-options-page`
		);
		await expect(
			page.getByText('Options Pages', { exact: true }).first()
		).toBeVisible();
		await page
			.locator('.acf-headerbar')
			.getByRole('link', { name: /add new/i })
			...
			.click();
		await expect(
			page.getByRole('heading', { name: 'Add New Options Page' }).first()
		).toBeVisible();

		await page
			.locator('label[for="acf_ui_options_page-advanced_configuration"]')
			.click();
		await expect(page.locator('#acf-advanced-settings')).toBeVisible();
		await page
			.locator('#acf-advanced-settings')
		...
		await expect(
			page.locator('#acf_ui_options_page-show_in_graphql')
		).not.toBeChecked();
		await page
			.locator('input[name="acf_ui_options_page[page_title]"]')
			.fill('Test Options Page');
		await page
			.locator('input[name="acf_ui_options_page[menu_slug]"]')
			.fill('test-options-page');
		await page
			.locator('select[name="acf_ui_options_page[parent_slug]"]')
			.selectOption('none');
		await page
			.locator('label[for="acf_ui_options_page-show_in_graphql"]')
			.click();
		...
		await expect(
			page.locator('input[name="acf_ui_options_page[page_title]"]')
		).toHaveValue('Test Options Page');
		await expect(
			page.locator('input[name="acf_ui_options_page[graphql_type_name]"]')
		).toHaveValue('TestOptionsPage');

		await visitAdminFacingPage(page, TOOLS_URL);
		await expect(page.getByText('Select Options Pages')).toBeVisible();
		await expect(
			page
				.locator('[data-name="ui_options_page_keys"]')
				.getByText('Test Options Page')
		).toBeVisible();
		await page
			.locator('[data-name="ui_options_page_keys"]')
			.getByRole('checkbox', { name: 'Test Options Page' })
			.check();
		await page
			.locator('#acf-admin-tool-export')
			.getByRole('button', { name: 'Generate PHP' })
		...
		expect(phpContent).toContain(
			"'graphql_type_name' => 'TestOptionsPage'"
		);
	});
});
