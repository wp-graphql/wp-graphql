# Manual testing guide for WPGraphQL Persisted Query URLs
...
`npm run -w @wpgraphql/wp-graphql-pqu test:codecept:wpunit` (and `test:codecept:functional` when present).

## Prerequisites

1. **WordPress environment running** (via `wp-env` or similar)
2. **WPGraphQL** and **WPGraphQL Smart Cache** plugins activated
3. **WPGraphQL Persisted Query URLs** plugin activated
...
## Important: Nonce-based persisted URL flow
...
- **`wp_wpgraphql_pqu_documents`**: Stores unique query documents (one row per unique `query_hash`)
...
- **`wp_wpgraphql_pqu_executions`**: One row per persisted **operation** (`query_hash` + `variables_hash`) for warm GET lookup. Survives cache-key purges; `last_executed_at` updates on successful warm GET (and on register via `store()`).

- **`wp_wpgraphql_pqu_urls`**: One row per persisted URL path (`url`, `url_hash`, `query_hash`, `variables_hash`, `last_seen_at` for GC)
- **`wp_wpgraphql_pqu_cache_keys`**: Deduplicated Smart Cache key strings (`id`, `cache_key` unique)
- **`wp_wpgraphql_pqu_key_urls`**: Junction `(key_id, url_id)` composite PRIMARY KEY linking `cache_keys.id` to `urls.id`
...
The plugin registers a **`wp graphql-pqu register`** command that performs the same steps as the cold GET + POST handshake: it generates a one-time nonce, runs your query through `graphql()` with Persisted Query URLs protocol extensions, and persists the document / execution when analyzer keys exist.

```bash
npm run wp-env -- run cli -- wp graphql-pqu register --query='query { posts { nodes { id } } }'
npm run wp-env -- run cli -- wp graphql-pqu register path/to/query.graphql --variables='{"id":"cG9zdDox"}'
npm run wp-env -- run cli -- wp graphql-pqu register --help
```
...
## Test 1: Complete persisted URL flow - Store Query in Index (with Nonce)

This test demonstrates the complete persisted URL flow from start to finish. The flow requires:
...
```sql
SELECT * FROM wp_wpgraphql_pqu_documents ORDER BY created_at DESC LIMIT 1;
```
...
**Expected Result:**
- **In `wp_wpgraphql_pqu_documents`**: A new row with:
...
- **Key map** (only when the POST is **not** logged in, unless you override `wpgraphql_pqu_should_record`): one row in `wp_wpgraphql_pqu_urls` for the path, one row per distinct `cache_key` in `wp_wpgraphql_pqu_cache_keys`, and matching rows in `wp_wpgraphql_pqu_key_urls`.
...
- Query document is stored in `wp_wpgraphql_pqu_documents` (normalized, stored once)
- URL ↔ cache key associations live in `wp_wpgraphql_pqu_urls` + `wp_wpgraphql_pqu_cache_keys` + `wp_wpgraphql_pqu_key_urls` (see join query above)
...
- JSON error response with:
  ...
  - `extensions.persistedQueryNonce`: Nonce token for the persisted URL registration flow
...
Here's a complete bash script that tests the full persisted URL flow for a query without variables:

```bash
#!/bin/bash

# Complete persisted URL flow test script
...
echo ""
echo "✓ Complete persisted URL flow test successful!"
```

### Script 2: Query With Variables

Here's a complete bash script that tests the full persisted URL flow for a query **with variables**:

```bash
#!/bin/bash

# Complete persisted URL flow test script (with variables)
...
QUERY_HASH=$(npm run wp-env -- run cli -- wp eval 'require_once "wp-content/plugins/wp-graphql-pqu/vendor/autoload.php"; use WPGraphQL\PQU\Utils\Hasher; echo Hasher::hash_query("query GetPost(\$id: ID!) { post(id: \$id) { id title } }");' 2>/dev/null | tail -1)

VARIABLES_HASH=$(npm run wp-env -- run cli -- wp eval 'require_once "wp-content/plugins/wp-graphql-pqu/vendor/autoload.php"; use WPGraphQL\PQU\Utils\Hasher; $vars = ["id" => "cG9zdDox"]; echo Hasher::hash_variables($vars);' 2>/dev/null | tail -1)
...
echo ""
echo "✓ Complete persisted URL flow with variables test successful!"
```

Save these scripts as `test-pqu-flow.sh` and `test-pqu-flow-variables.sh`, make them executable (`chmod +x test-pqu-flow*.sh`), and run them to test the complete flows.

## Test 3: Cache Invalidation - Purge Handler

This test verifies that this plugin correctly identifies and logs URLs that should be purged when WPGraphQL Smart Cache emits purge events.
...
2. **Optional: Prevent database entry deletion during testing:**
   ...
   add_filter( 'wpgraphql_pqu_delete_entries_on_purge', '__return_false' );
   ```
...
This plugin does **not** write routine purge traces to `error_log`. To log them locally, temporarily add:

```php
add_action(
	'wpgraphql_pqu_purge_resolved',
	static function ( $cache_key, $event, $hostname, $urls ) {
		...
		error_log(
			sprintf(
				'[PQU purge] key=%s event=%s host=%s urls=%s',
				$cache_key,
				$event,
				$hostname,
				wp_json_encode( $urls )
...
**Expected Results:**
- Each event should fire `graphql_purge` with different cache keys (observe via `wpgraphql_pqu_purge_resolved` or Smart Cache’s own purge logging if enabled):
...
- The **execution** row in `wp_wpgraphql_pqu_executions` for that query/variables remains so the persisted URL still resolves at origin after an edge MISS.
...
```php
add_filter( 'wpgraphql_pqu_allow_authenticated', '__return_true' );
```
...
```php
add_filter( 'wpgraphql_pqu_url_base', function() {
...
Hook `wpgraphql_pqu_purge_resolved` if you need purge traces in logs; `NullAdapter` does not log by itself.
...
SELECT COUNT(*) FROM wp_wpgraphql_pqu_documents;

-- Count junction rows (URL ↔ key associations)
SELECT COUNT(*) FROM wp_wpgraphql_pqu_key_urls;
...
FROM wp_wpgraphql_pqu_documents d
INNER JOIN wp_wpgraphql_pqu_urls u ON u.query_hash = d.query_hash
INNER JOIN wp_wpgraphql_pqu_key_urls ku ON ku.url_id = u.id
INNER JOIN wp_wpgraphql_pqu_cache_keys k ON k.id = ku.key_id
...
SELECT u.* FROM wp_wpgraphql_pqu_urls u WHERE u.url = '/graphql/persisted/...';

-- Find the document for a specific query hash
SELECT * FROM wp_wpgraphql_pqu_documents WHERE query_hash = '...';
```
...
**Solution**: Follow the proper persisted URL flow:
...
4. Performance testing with many persisted queries — see **[benchmark/README.md](./benchmark/README.md)** for Varnish in front of wp-env, `WPGRAPHQL_PQU_HTTP_PURGE_ORIGIN` + `HttpPurgeAdapter`, and optional k6 runs
