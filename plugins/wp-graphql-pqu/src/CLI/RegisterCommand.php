<?php
/**
 * WP-CLI: register persisted queries (URL index + document) without manual curl/POST.
 *
 * @package WPGraphQL\PQU\CLI
 * @since 0.1.0-beta.1
 */

namespace WPGraphQL\PQU\CLI;

use GraphQLRelay\Relay;
use WPGraphQL\PQU\App;
...
class RegisterCommand extends \WP_CLI_Command {
	 ...
	 *
	 * Generates a one-time nonce, runs the query through `graphql()` with Persisted Query URLs extensions, and
	 ...
	 * ## EXAMPLES
	 *
	 *     wp graphql-pqu register query.graphql
	 *     wp graphql-pqu register --query='query { posts { nodes { id } } }'
	 *     wp graphql-pqu register single-post.graphql --variables='{"id":"cG9zdDox"}'
	 *     wp graphql-pqu register query.graphql --edge-base=http://localhost:8081
	 *
	...
	public function register( array $args, array $assoc_args ): void {
		...
		if ( ! $app->can_load_plugin() ) {
			\WP_CLI::error( 'WPGraphQL Persisted Query URLs requires WPGraphQL and WPGraphQL Smart Cache to be active.' );
		}
	 ...
	 * ## EXAMPLES
	 *
	 *     wp graphql-pqu bulk-register benchmark/k6/single-post.graphql --limit=200 --urls-out=/tmp/urls.txt --manifest-out=/tmp/manifest.json
	 *     wp graphql-pqu bulk-register q.graphql --variables-jsonl=vars.jsonl --urls-out=urls.txt
	 *
	...
	private function register_and_get_path( string $query, array $variables ): array {
		...
		if ( ! defined( 'WPGRAPHQL_PQU_INTERNAL_CALL' ) ) {
			define( 'WPGRAPHQL_PQU_INTERNAL_CALL', true );
		}
