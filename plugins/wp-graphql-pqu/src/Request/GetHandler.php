 ...
 *
 * @package WPGraphQL\PQU\Request
 * @since 0.1.0-beta.1
 */

namespace WPGraphQL\PQU\Request;
...
class GetHandler {
	...
	public function handle( string $query_hash, ?string $variables_hash ): void {
		try {
			...
			error_log( '[WPGraphQL PQU] GetHandler: ' . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine() );
			...
			$message = ( defined( 'WP_DEBUG' ) && WP_DEBUG ) ? $e->getMessage() : __( 'Persisted query request failed.', 'wp-graphql-pqu' );
	...
	private function do_handle( string $query_hash, ?string $variables_hash ): void {
		...
		if ( ! is_string( $query_document ) || '' === $query_document ) {
			$this->send_document_invalid_response( __( 'Stored persisted query document is missing. Register the query again with a POST to /graphql.', 'wp-graphql-pqu' ) );
		}
		...
		try {
			...
			$message = __( 'Stored persisted query document is invalid. Register the query again with a POST to /graphql.', 'wp-graphql-pqu' );
			if ( '' !== $detail ) {
				$message .= ' ' . $detail;
			}
	...
	private function set_cache_headers(): void {
		...
		if ( null === $max_age ) {
			 ...
			 * @return int|null
			 */
			$max_age = apply_filters( 'wpgraphql_pqu_cache_max_age', 600 );
		}
