...
class PostHandler {
	...
	public function handle_post_response( $filtered_response, $schema, $operation, $query, $variables, $request, $query_id ) {
		...
		$is_internal_call = defined( 'WPGRAPHQL_PQU_INTERNAL_CALL' ) && WPGRAPHQL_PQU_INTERNAL_CALL;
		...
		// Check if nonce is provided in request extensions (for persisted URL registration validation).
		...
		if ( ! $document_exists ) {
			 ...
			 * Default is true (nonce required for persisted URL registration security).
			 ...
			 * @return bool
			 */
			$require_nonce = apply_filters( 'wpgraphql_pqu_require_nonce', true );
		...
		if ( $should_store_execution_data ) {
			 ...
			 * @return bool
			 */
			$record_cache_tags = (bool) apply_filters( 'wpgraphql_pqu_should_record', ! is_user_logged_in(), $url, $cache_keys, $request );
	...
	private function build_url( string $query_hash, ?string $variables_hash ): string {
		 ...
		 * @return string
		 */
		$base_path = apply_filters( 'wpgraphql_pqu_url_base', 'graphql/persisted/' );
