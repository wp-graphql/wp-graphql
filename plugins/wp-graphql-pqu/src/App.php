...
class App {
	...
	private function get_plugin_load_error_messages(): array {
		...
		if ( ! class_exists( 'WPGraphQL' ) ) {
			$messages[] = __( 'WPGraphQL must be installed and activated', 'wp-graphql-pqu' );
		}

		if ( ! defined( 'WPGRAPHQL_SMART_CACHE_VERSION' ) ) {
			$messages[] = __( 'WPGraphQL Smart Cache must be installed and activated', 'wp-graphql-pqu' );
		}
			...
			static function () use ( $messages ) {
				...
				<div class="error notice">
					<h3>
						...
						echo esc_html( sprintf( __( 'WPGraphQL Persisted Query URLs v%s cannot load', 'wp-graphql-pqu' ), WPGRAPHQL_PQU_VERSION ) );
						?>
					</h3>
					<ol>
	...
	public function show_graphql_debug_messages(): void {
		...
		$prefix = sprintf( 'WPGraphQL Persisted Query URLs v%s cannot load', WPGRAPHQL_PQU_VERSION );
	...
	public function activate(): void {
		...
		Schema::update_db_version( WPGRAPHQL_PQU_VERSION );

		// Flush rewrite rules to register new routes.
		flush_rewrite_rules(); // phpcs:ignore WordPressVIPMinimum.Functions.RestrictedFunctions.flush_rewrite_rules_flush_rewrite_rules -- Activation: register persisted URL routes.
	...
	public function deactivate(): void {
		...
		wp_clear_scheduled_hook( 'wpgraphql_pqu_garbage_collection' );

		// Flush rewrite rules.
		flush_rewrite_rules(); // phpcs:ignore WordPressVIPMinimum.Functions.RestrictedFunctions.flush_rewrite_rules_flush_rewrite_rules -- Deactivation: remove persisted URL routes.
	}
	...
	private function schedule_garbage_collection(): void {
		if ( ! wp_next_scheduled( 'wpgraphql_pqu_garbage_collection' ) ) {
			wp_schedule_event( time(), 'daily', 'wpgraphql_pqu_garbage_collection' );
		}
	}
}
