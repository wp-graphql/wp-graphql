 ...
 * define WPGRAPHQL_PQU_HTTP_PURGE_ORIGIN (e.g. http://host.docker.internal:8081).
 *
...
use WPGraphQL\PQU\Utils\Logger;
...
class HttpPurgeAdapter implements AdapterInterface {
	...
	public static function is_available(): bool {
		if ( ! defined( 'WPGRAPHQL_PQU_HTTP_PURGE_ORIGIN' ) ) {
			return false;
		}

		$origin = constant( 'WPGRAPHQL_PQU_HTTP_PURGE_ORIGIN' );
	...
	public function purge_url( string $url ): bool {
		 ...
		 * @param string $url    Original path or URL from this plugin.
		 * @return string
		 */
		$method = apply_filters( 'wpgraphql_pqu_http_purge_method', 'PURGE', $target, $url );
		 ...
		 */
		$args = apply_filters(
			'wpgraphql_pqu_http_purge_request_args',
			[
	...
	/**
	 * Build full edge URL from persisted path or absolute site URL.
	 *
	...
	private function build_edge_url( string $url ): string {
		...
		$origin_raw = constant( 'WPGRAPHQL_PQU_HTTP_PURGE_ORIGIN' );
