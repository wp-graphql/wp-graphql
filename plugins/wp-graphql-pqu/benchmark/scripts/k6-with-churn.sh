...
#   ./plugins/wp-graphql-pqu/benchmark/scripts/k6-with-churn.sh   # from monorepo root
...
# Do not match workspace packages that only mention wp-env inside other script strings (e.g. plugins/wp-graphql-pqu).
...
echo "Expect pqu_x_cache_misses to bump after each save when URLs in urls.txt tag that post; Ctrl+C stops both."
