# Persisted Query URLs — edge-cache benchmarking

This folder helps you reproduce **URL-keyed edge caching** (Varnish) in front of WordPress, plus **HTTP PURGE** invalidation for persisted query paths—matching how **WPGraphQL Persisted Query URLs** is meant to run on hosts that cache GETs by URL.
...
**Recording hypothesis runs:** copy [RESULTS.template.md](./RESULTS.template.md) per scenario and paste k6 output + manifest knobs. Keep finished `results/*.json` / `results/*.md` **local only** (gitignored); see [results/.gitignore](./results/.gitignore).

**Persisted Query URLs MySQL index (for DB checks during benches):** documents + executions for warm GET; **normalized key map** = `wpgraphql_pqu_urls`, `wpgraphql_pqu_cache_keys`, `wpgraphql_pqu_key_urls` (replaces legacy `wpgraphql_pqu_url_keys`). Details in [../docs/INTEGRATIONS.md](../docs/INTEGRATIONS.md).

## Prerequisites

- [wp-env](https://developer.wordpress.org/block-editor/reference-guides/packages/packages-env/) (or any WordPress at `http://localhost:8888`) with **WPGraphQL**, **WPGraphQL Smart Cache**, and **wp-graphql-pqu** active.
- Docker (for Varnish).
...
```bash
cd plugins/wp-graphql-pqu/benchmark
docker compose up -d
```
...
```bash
docker compose -f plugins/wp-graphql-pqu/benchmark/docker-compose.yml up -d
```
...
**Two “hosts” to keep straight:** Clients and k6 talk to the **edge** port. PHP inside wp-env purges the edge using **`WPGRAPHQL_PQU_HTTP_PURGE_ORIGIN`** (see §2), which must point at Varnish from **inside** the WordPress container (`host.docker.internal:8081`), not at `:8888`.
...
## 2. Set `WPGRAPHQL_PQU_HTTP_PURGE_ORIGIN` (WordPress → edge)
...
```json
{
  "env": {
    "development": {
      "config": {
        "WPGRAPHQL_PQU_HTTP_PURGE_ORIGIN": "http://host.docker.internal:8081"
      }
    }
  }
}
...
```bash
npm run wp-env run cli -- wp config set WPGRAPHQL_PQU_HTTP_PURGE_ORIGIN http://host.docker.internal:8081 --type=constant
```
...
With `WP_DEBUG` enabled, failed purges log `[WPGraphQL PQU] HttpPurgeAdapter:` in `debug.log`. You can override `wp_remote_request` with the filter `wpgraphql_pqu_http_purge_request_args` (for example `sslverify` or headers).
...
Avoid manual cold-GET + POST: use **`wp graphql-pqu register`** (ships with this plugin). From the monorepo root with wp-env running:

```bash
npm run wp-env -- run cli -- wp graphql-pqu register \
...
```bash
npm run wp-env -- run cli -- wp graphql-pqu register path/to/query.graphql --variables-file=vars.json
```
...
See `wp graphql-pqu register --help` for options (`--user`, STDIN `-`, etc.). Manual flow remains in [../TESTING.md](../TESTING.md).
...
Use **`wp graphql-pqu bulk-register`** (hyphen; **`bulk_register`** with an underscore is an alias) to register the same document with many variable payloads (newspaper-style long tail). Paths are written for k6 (one per line, no host). **`--limit=200` only creates as many URLs as you have matching posts** (e.g. three posts → three paths).

**Inside wp-env**, use paths under `/var/www/html/wp-content/plugins/wp-graphql-pqu/benchmark/k6/` (or your mounted plugin path).

```bash
# Register one persisted URL per published post (default limit 100), write paths + manifest.
npm run wp-env -- run cli -- wp graphql-pqu bulk-register \
  /var/www/html/wp-content/plugins/wp-graphql-pqu/benchmark/k6/single-post.graphql \
  --limit=200 \
  --urls-out=/var/www/html/wp-content/plugins/wp-graphql-pqu/benchmark/k6/urls.txt \
  --manifest-out=/var/www/html/wp-content/plugins/wp-graphql-pqu/benchmark/k6/run-manifest.json \
  --manifest-template=/var/www/html/wp-content/plugins/wp-graphql-pqu/benchmark/k6/run-manifest.example.json \
  --edge-base=http://localhost:8081
```
...
2. **Build persisted URLs** (emits JSONL under `k6/generated/`, runs `bulk-register` per template, writes shuffled `k6/urls-headless-day.txt`):
   ...
   ```bash
   ./plugins/wp-graphql-pqu/benchmark/scripts/build-persisted-urls.sh
   ```
...
4. **Churn** — [scripts/k6-with-realistic-churn.sh](./scripts/k6-with-realistic-churn.sh) rotates new posts, random title updates, category/tag `post term set`, and **Primary Nav** menu items (no post deletes, so singular URLs in the list keep returning 200).
   ...
   ```bash
   URLS_FILE=urls-headless-day.txt ./plugins/wp-graphql-pqu/benchmark/scripts/k6-with-realistic-churn.sh
   ```
...
These scripts **`cd` into `benchmark/k6` themselves**, so k6 always finds `urls.txt` and `pqu-persisted-get.js`. You still must invoke the **`.sh` path** from your shell’s current directory (or use an absolute path).
...
```bash
chmod +x plugins/wp-graphql-pqu/benchmark/scripts/run-k6-edge.sh
chmod +x plugins/wp-graphql-pqu/benchmark/scripts/run-k6-origin.sh

DURATION=30s VUS=5 ./plugins/wp-graphql-pqu/benchmark/scripts/run-k6-edge.sh
DURATION=30s VUS=5 REQUIRE_X_CACHE=1 ./plugins/wp-graphql-pqu/benchmark/scripts/run-k6-edge.sh

DURATION=30s VUS=5 ./plugins/wp-graphql-pqu/benchmark/scripts/run-k6-origin.sh
```

From **`plugins/wp-graphql-pqu/benchmark/k6`** (e.g. after `cd` there for raw `k6 run`):
...
**Working directory:** If you prefer raw `k6 run`, the `cd plugins/wp-graphql-pqu/benchmark/k6` line below assumes the **monorepo root**. If you are **already** in `benchmark/k6`, run `k6 run pqu-persisted-get.js ...` only.

**Local k6** (after `brew install k6` or another [install](https://grafana.com/docs/k6/latest/set-up/install-k6/)):

```bash
cd plugins/wp-graphql-pqu/benchmark/k6
k6 run pqu-persisted-get.js -e BASE_URL=http://localhost:8081 -e URLS_FILE=urls.txt
```

**Edge-only checks:** to fail the run if `X-Cache` is missing (Varnish not in path), add `-e REQUIRE_X_CACHE=1`.

**Minimum hit rate** (after warm-up, edge only): `-e MIN_HIT_RATE=0.85` adds a threshold on `pqu_x_cache_hit_rate` (0–1). Omit on cold cache or origin runs.

**Origin baseline (no edge):** same script, `BASE_URL=http://localhost:8888`. Expect **`pqu_x_cache_unknown`** to dominate (no `X-Cache` from WordPress). Do **not** set `REQUIRE_X_CACHE` or `MIN_HIT_RATE`.

### Reading k6 output (custom metrics)

| Metric | Meaning |
|--------|---------|
| `pqu_x_cache_hits` / `pqu_x_cache_misses` | Count of responses whose `X-Cache` starts with `HIT` / `MISS` (bundled VCL: `HIT: N` or `MISS`). |
| `pqu_x_cache_unknown` | No header or unrecognized value (typical on **origin-only** runs). |
| `pqu_x_cache_hit_rate` | `Rate`: fraction of iterations classified as HIT; summary shows `rate=…` (use with `MIN_HIT_RATE`). |
| `pqu_get_duration` | Client-side request duration (trend). |
...
| `BASE_URL` | Throughput | `pqu_x_cache_*` | Checks |
|------------|------------|-----------------|--------|
...
[scripts/k6-with-churn.sh](./scripts/k6-with-churn.sh) runs **`k6` against the edge** in the background and **updates a post** on an interval so Smart Cache emits **`graphql_purge`** and the plugin purges Varnish. You should see **`pqu_x_cache_misses`** increase around churn ticks (then hits recover as the edge refills). For **list + archive + menu** invalidation, use [scripts/k6-with-realistic-churn.sh](./scripts/k6-with-realistic-churn.sh) with `urls-headless-day.txt` (see §5b).
...
**`WP_WORKDIR`:** If you use `WP_BIN='npm run wp-env -- run cli -- wp'`, the script walks upward for a `package.json` that defines the **`wp-env` npm script** (`"wp-env": "wp-env"`), i.e. the monorepo root—not a workspace package that only references wp-env inside other scripts. Override if needed: `WP_WORKDIR=/path/to/wp-graphql`.

From **monorepo root**:

```bash
chmod +x plugins/wp-graphql-pqu/benchmark/scripts/k6-with-churn.sh
export WP_BIN='npm run wp-env -- run cli -- wp'
POST_ID=10 CHURN_INTERVAL=25 DURATION=2m VUS=10 \
  ./plugins/wp-graphql-pqu/benchmark/scripts/k6-with-churn.sh
```

From **`plugins/wp-graphql-pqu/benchmark/k6`**:
...
[scripts/pqu-churn-sample.sh](./scripts/pqu-churn-sample.sh) updates a post on a timer and `curl -sI`s **one** persisted URL so you can watch **`X-Cache`** in the shell (no k6).

```bash
chmod +x plugins/wp-graphql-pqu/benchmark/scripts/pqu-churn-sample.sh
export WP_BIN='npm run wp-env -- run cli -- wp'
