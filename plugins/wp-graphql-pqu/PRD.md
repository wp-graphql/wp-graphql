...
added via the `wpgraphql_pqu_purge_adapter` filter.
...
**Table 1: `wp_wpgraphql_pqu_documents`**

Stores unique query documents (one row per unique query hash):

> CREATE TABLE {prefix}wpgraphql_pqu_documents (
>
...
**Table 2: `wp_wpgraphql_pqu_executions`**
...
-   `{prefix}wpgraphql_pqu_urls` — `id`, `url`, `url_hash` (unique),
    `query_hash`, `variables_hash`, `last_seen_at`
-   `{prefix}wpgraphql_pqu_cache_keys` — `id`, `cache_key` (unique)
-   `{prefix}wpgraphql_pqu_key_urls` — `key_id`, `url_id` (composite
    primary key)

Early internal builds used a denormalized `{prefix}wpgraphql_pqu_url_keys`
...
**6.3 Storage Backend by Host**
  ...
  ---------------------- ------------------ ------------------------------
  **Environment**        **Object Cache**   **PQU index store**
...
> interface WPGraphQL_PQU_Purge_Adapter {
>
...
**Phase 1.6 --- PQU flow with nonce** 📋 (Planned)
...
*WPGraphQL Persisted Query URLs — Product Requirements Document — March 2026*