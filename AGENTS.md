...
This file provides guidance to AI coding agents (Claude Code, Codex, Cursor, and others) when working in this repository. Claude Code reads it through the thin `CLAUDE.md` pointer files, which import the nearest `AGENTS.md`. It covers the monorepo as a whole and the conventions shared across plugins. Each plugin has its own `AGENTS.md` (linked below) with plugin-specific commands, architecture, and gotchas — those load automatically when you work inside the plugin's directory.
...
- **plugins/wp-graphql/** — Core GraphQL API plugin (PHP 7.4+, WP 6.0+) — [`AGENTS.md`](plugins/wp-graphql/AGENTS.md)
- **plugins/wp-graphql-ide/** — GraphiQL IDE for WordPress admin (React-based) — [`AGENTS.md`](plugins/wp-graphql-ide/AGENTS.md)
- **plugins/wp-graphql-smart-cache/** — Caching and cache invalidation — [`AGENTS.md`](plugins/wp-graphql-smart-cache/AGENTS.md)
- **plugins/wp-graphql-acf/** — ACF field groups and fields in GraphQL — [`AGENTS.md`](plugins/wp-graphql-acf/AGENTS.md)
...
Tests and PHP linting run inside the wp-env Docker containers and are invoked per workspace. The exact scripts, file-path conventions, and single-test syntax differ per plugin — see that plugin's `AGENTS.md`. Workspace names:
...
These apply across the PHP plugins; see each plugin's `AGENTS.md` for its specifics (PHP floor, PHPStan level, JS tooling).
