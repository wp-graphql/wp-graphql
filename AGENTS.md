...
- **Testing**: Prefer WPUnit first (we have code coverage; WPUnit runs in WordPress so integration-style). Use acceptance/E2E for UI or browser behavior (JavaScript). For tricky UI, Playwright and its UI mode are helpful (js-e2e tests).
...
3. Run smoke tests: `./bin/smoke-test.sh` (optionally `--verbose`).
4. Run relevant unit or E2E tests as needed.
...
Project-specific procedures live in `.ai/skills/`. Invoke them when the task matches:
