...
run_app_xdebug() {
  env DOCKER_HOST_IP="$(get_docker_host_os_ip)" docker-compose -f docker/docker-compose.local-app-xdebug.yml up --build
}

cleanup_docker_artifacts() {
  docker-compose -f docker/docker-compose.local-app-xdebug.yml down -v --rmi local 2> /dev/null
}
