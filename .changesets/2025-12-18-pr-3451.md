...
- 🚨🚨🚨 The variables inside \`.env(.dist)\` are now sorted and also used consistently: prefixed \`TEST_\` are used by codeception, nonprefixed are used by \`install-test-env.sh\`. (Later versions of codeception support variable inheritance in \`.env\`)
