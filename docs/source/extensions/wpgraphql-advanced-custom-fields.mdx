...
Now if you want to expose an ACF field group, make sure to set the `GraphQL Field Name` in the settings of the field group. To be consistent with the schema, make sure it is written in `camelCase`.
...
So lets say we have the following ACF structure with a flexible content field:
...
In the response, `flexibleContents` will be an array with whatever type of layout (flexible content field) has been added to the page. With the `fieldGroupName` you can then figure out, what type of layout it is and treat it in accordingly.
