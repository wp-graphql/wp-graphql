...
export default function ThemeToggle({ panelClassName = "mt-4" }) {
  ...
  return (
    <Listbox value={setting} onChange={setSetting}>
      ...
      <Listbox.Options
        className={clsx(
          "absolute z-50 top-full max-md:right-[60px] bg-white rounded-lg ring-1 ring-slate-900/10 shadow-lg overflow-hidden w-36 py-1 text-sm text-slate-700 font-semibold dark:bg-slate-900 dark:ring-0 dark:highlight-white/5 dark:text-slate-300 -mt-5 dark:shadow-lg",
          panelClassName
        )}
      >
